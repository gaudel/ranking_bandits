...
class propos_Round_Robin():
    ...
    def get_proposal(self):
        if self.proposal_type_at_this_turn == 'TGRW':
            ...
            TruncatedGaussianRandomWalk(on_theta=False, sigma=self.sigma, k=i) for i in range(1, self.nb_position)]
            ...
            return [Proposal_RW_logit(on_theta=True, sigma=self.sigma, k=i) for i in range(self.nb_arms)] + [
                Proposal_RW_logit(on_theta=False, sigma=self.sigma, k=i) for i in range(1, self.nb_position)]
            ...
            return [Proposal_pseudoViewBis(on_theta=True, dico=self.dico, k=i) for i in range(self.nb_arms)] + [
                Proposal_pseudoViewBis(on_theta=False, dico=self.dico, k=i) for i in range(1, self.nb_position)]
            ...
            return [Proposal_maxposition(on_theta=True, dico=self.dico, k=i) for i in range(self.nb_arms)] + [
                Proposal_maxposition(on_theta=False, dico=self.dico, k=i) for i in range(1, self.nb_position)]
